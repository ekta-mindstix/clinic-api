...
def test_list_and_delete_availability_service(app):
    doctor = DoctorService.onboard_doctor(
        email="availability-doctor@test.local",
        full_name="Availability Doctor",
        password="Pass@123",
        license_number="LIC-SVC-5",
    )

    start = datetime.now().replace(second=0, microsecond=0) + timedelta(days=2)
    end = start + timedelta(hours=1)
    created = DoctorService.create_availability(
        doctor_user_id=doctor["user_id"],
        start_at=start,
        end_at=end,
    )

    availabilities = DoctorService.list_my_availability(doctor_user_id=doctor["user_id"])
    assert len(availabilities) == 1
    assert availabilities[0]["id"] == created["id"]

    deleted = DoctorService.delete_availability(
        doctor_user_id=doctor["user_id"],
